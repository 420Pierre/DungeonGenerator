	...
	for p in hallway_graph.get_point_ids():
		for c in hallway_graph.get_point_connections(p):
			if c > p:
				...
				# Tür-Positionen korrekt setzen (wichtig: Vector3 → Vector3i!)
				var v3_from = Vector3i(tile_from.x, 0, tile_from.z)
				var v3_to = Vector3i(tile_to.x, 0, tile_to.z)
				grid_map.set_cell_item(v3_from, 2)
				grid_map.set_cell_item(v3_to, 2)
...
func generate_room(rec: int):
	...
	start_pos.x = randi() % (border_size - width + 1) #+1 nötig, weil nie gleich border sein kann
