...
func generate():
	...
	for p in room_positions:
		...
		mst_graph.add_point(mst_graph.get_available_point_id(), Vector2(p.x,p.z)) #punkte bisher nur platziert, sind aber noch nicht verbunden
		
	var delaunay : Array = Array(Geometry2D.triangulate_delaunay(rpv2))
		
	for i in delaunay.size()/3: #einfach hardcoden, weil es in einem dreieck nur drei punkte gibt
	...
	generate_hallways(hallway_graph)
	
func generate_hallways(hallway_graph:AStar2D):
	var hallways : Array[PackedVector3Array] = []
	for p in hallway_graph.get_available_point_ids():
...
func generate_room(rec:int):
	...
	starting_position.x = randi() % (border_size - width + 1) #+1 nötig, weil nie gleich border sein kann
	starting_position.z = randi() % (border_size - height + 1)
	
	for r in range(-room_margin, height + room_margin): #damit Räume nicht überlappen
		for c in range(-room_margin, width + room_margin):
			var pos : Vector3i = starting_position + Vector3i(c,0,r)
			if grid_map.get_cell_item(pos) == 0:
				generate_room(rec-1)
				return
	...
	var avg_x : float = starting_position.x + (float(width)/2)
	var avg_z : float = starting_position.z + (float(height)/2)
	var pos : Vector3 = Vector3(avg_x, 0, avg_z)
	room_positions.append(pos) #machen zu Vector3 und fügen es dann oben zum Array hinzu
